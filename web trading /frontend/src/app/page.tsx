...
export default function Dashboard() {
  ...
  const fetchStockData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_BASE_URL}/api/stock/${selectedSymbol}`);
  ...
  }, [selectedSymbol, setStockData, setLoading]);
  ...
  const handleSentimentAnalysis = async () => {
    ...
    try {
      const res = await axios.post(
        `${API_BASE_URL}/api/sentiment?text=${encodeURIComponent(newsText)}`
      );
      ...
      <header className="flex justify-between items-center mb-8 border-b border-bloomberg-border pb-4">
        ...
        <div className="flex gap-4">
          ...
          <button
            ...
            className="bg-bloomberg-accent text-black px-4 py-2 rounded font-bold hover:bg-yellow-600"
          >
            LOAD
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          >
            <div className="flex justify-between mb-4">
              <h2 className="text-xl font-bold">
                {stockData.symbol} - IDR {stockData.current_price.toLocaleString()}
              </h2>
              <span
                className={`font-bold ${
            ...
            <div className="grid grid-cols-3 gap-4 mt-4 text-sm">
              <div className="bg-black/30 p-2 rounded">
                ...
                <div
                  className={
                    stockData.indicators.rsi_14! > 70
                      ? 'text-bloomberg-bear'
                      : 'text-bloomberg-bull'
                  }
                >
          ...
          <div className="space-y-6">
            ...
            <div className="bg-bloomberg-panel border border-bloomberg-border rounded-lg p-4">
              <h3 className="text-lg font-bold mb-2 text-bloomberg-accent">
            ...
            <div className="bg-bloomberg-panel border border-bloomberg-border rounded-lg p-4">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-bloomberg-bear">
                  Lower: {stockData.prediction_interval.lower_95.toFixed(0)}
                </span>
                <span className="text-bloomberg-bull">
                  Upper: {stockData.prediction_interval.upper_95.toFixed(0)}
                </span>
              </div>
              <p className="text-xs text-gray-500 mt-2">
                Based on historical volatility bootstrap.
              </p>
            </div>
          </div>
        </div>
